...
Nous allons représenter les données qu'on voit habituellement en algèbre linéaire et voir comment ceux-ci s'articulent dans le concepte de notre langage.
...
Bien qu'on puisse travailler avec des tenseurs de plusieurs dimensions, on se rend compte en réalité que les datascientistes travaillent le plus souvent avec des tenseurs allant jusqu'à la dimension 5 au maximum. En effet, prendre de plus grande dimensions rend les données difficile à interpréter. De plus, l'essentiel des opérations se réalise au niveau du calcul matriciel, le reste des dimension servant principalement de listes pour la représentation des informations.

On peut partir de l'hypothèse qu'un simple tableau (array) est un vecteur. Ce qui va nous permettre de sauté directement aux matrices.
...
L'un des éléments fondamentaux de l'algèbre linéaire son les matrices. En effet, les concepts de statistiques, de probabilité et de mathématiques sont représenté à l'aide de calcules sur les matrices. Le vecteur de vecteurs est la façon la plus simple de représenter les matrice. En effet les matrices ont une forme rectangulaire au carré ce qui fait que les sous vecteurs sont tous de la même taille ce qui va faciliter la représentation par des types.

Une matrice:

#Exemple($ mat(2, 2; 2, 2) $, caption: [C'est une matrice])

Peut-être remprésentée comme ceci:
...
Plus loin nous traitons la notion de broadcasting pas mal utilisé dans le domaine des sciences des données et vu comment celle-ci peut-être représentée avec notre système de type. Ici nous intéressons à des opérations simples et facile à typer. 
...
Pour ce faire, nous devons Définir la notion de mapping. Notre language est tiré du lambda calculus et exploite donc les notions de programmation fonctionnelle. Nous n'utilisons pas de boucle mais nous avons la notion de récursivité. Imaginons que nous voulions incrémenter de 1 tout les éléments d'un tableau avec notre langage. Le système de type nous prévient de faire des opération erronnées comme:
...
On est donc en mesure d'appeller la fonction de mapping avec cette fonction:

$ #proof-tree(eval("", "map(plus_1, [1, 2, 3, 4]) => [2, 3, 4, 5]", "")) $

$ #proof-tree(typing("", "map(plus_1, [1, 2, 3, 4]) : [4, int]")) $

Cette expression est correctement typé et donnera [2, 3, 4, 5].

Si nous voulons faire le même type d'opération sur les matrice (donc des tableaux de tableau), il va nous falloir définir une fonction de mappage d'un degré plus haut. En s'appuyant sur la définition de notre fonction map préalablement établie, on peut créer notre fonction map2 comme suite:
...
L'avantage de la définition choisie des tenseur se présente dans la similarité entre les deux fonction "map" et "map2". Faire la fonction map3 donnerait un résultat similaire, il suffirait juste de faire appel à "map3" à la place de "map2" et de "map2" à la place de "map1". Malheureusement notre langage ne supporte pas assez de fonctionnalité pour faire un mapping généralisé pour tout les tenseurs.

Une autre chose intéressante serait de pouvoir appliquer des opérations entres différents tenseurs. En algèbre linéaire, il faut que les matrices aient la même forme. Notre système de type peut assurer ça. On verra un peu plus loin le cas particulier du broadcasting. On aimerait être en mesure d'additionner ou de multiplier des matrice de même forme. Comme vu tout à l'heure, faire des calculs en utilisant directement l'opérateur ne marche pas et est prévenu par notre système de type:

$ #proof-tree(typing("", "[1, 2, 3, 4] + [4, 3, 2, 1] : ?")) $

Une solution serait de réapliquer la notion de mapping mais pour les fonctions binaires (à deux opérateur). Il y aurait un moyen de réutiliser les fonctions map définies précédement à l'aide de tuple, mais comme notre langage ne l'implémente pas, on se contentera de simplement créer des des fonctions binaires et une fonction "map_op".
...
On part du principe que la fonction "f" prend deux éléments du même type "T", mais on pourrait la généraliser d'avantage. On peut alors définir des fonctions binaire à l'aide des opérateurs de base.
...
Ce qui va nous intéresser maintenant et l'utilisation du produit matriciel. C'est une propriété fondamentale de l'algèbre linéaire qui nous servira à l'établissement de la librairie de réseaux de neurones. Si nous avons deux matrice $A_("MxP")$ et $B_("PxN")$ le produit matriciel A \* B donnera $C_("MxN")$. Par exemple:
